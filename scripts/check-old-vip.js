const admin = require('firebase-admin');

const oldServiceAccount = {
  "type": "service_account",
  "project_id": "bloodline-dna-nextjs",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  "client_email": "firebase-adminsdk-fbsvc@bloodline-dna-nextjs.iam.gserviceaccount.com"
};

if (admin.apps.length === 0) {
  admin.initializeApp({
    credential: admin.credential.cert(oldServiceAccount)
  });
}

const db = admin.firestore();

async function check() {
  const snap = await db.collection('users').where('email', '==', 'phanvanbin2321@gmail.com').get();
  snap.forEach(d => {
    const data = d.data();
    console.log('vipPlan:', data.vipPlan, typeof data.vipPlan);
    console.log('vipExpiresAt:', data.vipExpiresAt, typeof data.vipExpiresAt);
    if (data.vipExpiresAt && data.vipExpiresAt.toDate) {
       console.log('As Date:', data.vipExpiresAt.toDate());
    }
  });
  process.exit(0);
}
check();
