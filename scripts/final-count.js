...
async function countStats(projectTag, serviceAccount) {
  const app = admin.initializeApp({ credential: admin.credential.cert(serviceAccount) }, projectTag);
  const db = app.firestore();

  // Count VIP Users
  const uSnap = await db.collection('users').get();
  const vips = uSnap.docs.filter(d => {
    const data = d.data();
    return data.vipPlan && data.vipPlan !== '';
  });

  // Count VIP Upgrades
  const pSnap = await db.collection('payments').where('paymentFor', '==', 'vip_upgrade').where('status', '==', 2).get();
  
  console.log(`[${projectTag}] TÀI KHOẢN VIP: ${vips.length}`);
  console.log(`[${projectTag}] NÂNG CẤP VIP: ${pSnap.size}`);
}

async function main() {
  await countStats('old', oldServiceAccount);
  await countStats('new', newServiceAccount);
  process.exit(0);
}
