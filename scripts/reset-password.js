const admin = require('firebase-admin');
const bcrypt = require('bcryptjs');
const path = require('path');

const oldServiceAccount = {
  "type": "service_account",
  "project_id": "bloodline-dna-nextjs",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  "client_email": "firebase-adminsdk-fbsvc@bloodline-dna-nextjs.iam.gserviceaccount.com"
};

const newServiceAccount = require(path.join(process.cwd(), 'serviceAccountKey.json'));

async function reset(email, newPass) {
  const hash = await bcrypt.hash(newPass, 10);
  
  // Reset in OLD project
  const oldApp = admin.initializeApp({ credential: admin.credential.cert(oldServiceAccount) }, 'old');
  const oldDb = oldApp.firestore();
  const oldSnap = await oldDb.collection('users').where('email', '==', email).get();
  for (const d of oldSnap.docs) {
    await d.ref.update({ passwordHash: hash });
  }
  console.log('Reset OLD OK');

  // Reset in NEW project
  const newApp = admin.initializeApp({ credential: admin.credential.cert(newServiceAccount) }, 'new');
  const newDb = newApp.firestore();
  const newSnap = await newDb.collection('users').where('email', '==', email).get();
  for (const d of newSnap.docs) {
    await d.ref.update({ passwordHash: hash });
  }
  console.log('Reset NEW OK');
  
  process.exit(0);
}

reset('mongtrinh2003@gmail.com', '123456');
