const admin = require('firebase-admin');
const fs = require('fs');

// OLD Project Configuration
const oldServiceAccount = {
  "type": "service_account",
  "project_id": "bloodline-dna-nextjs",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  "client_email": "firebase-adminsdk-fbsvc@bloodline-dna-nextjs.iam.gserviceaccount.com"
};

const newServiceAccount = require('../serviceAccountKey.json');

// List of all collections to migrate
const collections = [
  'users',
  'testServices',
  'servicePrices',
  'testBookings',
  'testKits',
  'testSamples',
  'testResults',
  'payments',
  'blogs',
  'tags',
  'feedback',
  'sampleInstructions',
  'logistics',
  'otpCodes',
  'studyGroups',
  'groupMembers',
  'communityPosts',
  'communityComments',
  'mentorRequests',
  'mentorBookings',
  'mentorProfiles',
  'mentorReviews',
  'mentorCourses',
  'pomodoroSessions'
];

// Initialize Old Project App
const oldApp = admin.initializeApp({
  credential: admin.credential.cert(oldServiceAccount)
}, 'oldProject');

// Initialize New Project App
const newApp = admin.initializeApp({
  credential: admin.credential.cert(newServiceAccount)
});

async function migrateAll() {
  const oldDb = oldApp.firestore();
  const newDb = newApp.firestore();
  
  console.log('🚀 Bắt đầu quá trình di chuyển DỮ LIỆU TỔNG THỂ...');

  for (const collectionName of collections) {
    console.log(`\n📦 Đang xử lý Collection: [${collectionName}]`);
    
    const snapshot = await oldDb.collection(collectionName).get();
    if (snapshot.empty) {
      console.log(`  - Trống. Bỏ qua.`);
      continue;
    }

    const docs = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    console.log(`  - Tìm thấy ${docs.length} bản ghi.`);

    let successCount = 0;
    for (const docData of docs) {
      const { id, ...data } = docData;
      try {
        await newDb.collection(collectionName).doc(id).set(data);
        successCount++;
      } catch (err) {
        console.error(`  ❌ Lỗi tại ID ${id}:`, err.message);
      }
    }
    console.log(`  ✅ Hoàn tất: ${successCount}/${docs.length} được di chuyển.`);
  }

  console.log('\n--- KẾT QUẢ CUỐI CÙNG ---');
  console.log('🎉 TOÀN BỘ DỮ LIỆU ĐÃ ĐƯỢC CHUYỂN SANG DỰ ÁN MỚI THÀNH CÔNG!');
  process.exit(0);
}

migrateAll().catch(err => {
  console.error('❌ Lỗi hệ thống khi di chuyển:', err);
  process.exit(1);
});
