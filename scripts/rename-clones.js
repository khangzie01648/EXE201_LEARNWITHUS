const admin = require('firebase-admin');
const path = require('path');

const oldServiceAccount = {
  "type": "service_account",
  "project_id": "bloodline-dna-nextjs",
  "private_key": "-----BEGIN PRIVATE KEY-----\\\\n-----END PRIVATE KEY-----\n",
  "client_email": "firebase-adminsdk-fbsvc@bloodline-dna-nextjs.iam.gserviceaccount.com"
};

const newServiceAccount = require(path.join(process.cwd(), 'serviceAccountKey.json'));

const realNames = {
  '033093': 'Nguyễn Phương Thảo',
  '346436': 'Trần Anh Tuấn',
  '714982': 'Lê Thị Mai',
  '346634': 'Phạm Hồng Quân',
  '582103': 'Hoàng Gia Bảo',
  '636546': 'Vũ Tuyết Nhi',
  '658349': 'Phan Văn Hải',
  '551086': 'Bùi Bích Phương',
  'e48544': 'Đặng Thanh Tùng',
  'af3799': 'Ngô Mỹ Hạnh',
  'c47167': 'Trịnh Ngọc Duy',
  '557863': 'Đỗ Thùy Linh',
  '8c2994': 'Vương Kiến Quốc',
  'e18074': 'Lý Gia Thành',
  '060179': 'Tô Minh Hiếu',
  'vs1710': 'Đinh Thị Thủy'
};

async function renameAndCount(projectTag, serviceAccount) {
  const app = admin.initializeApp({ credential: admin.credential.cert(serviceAccount) }, projectTag);
  const db = app.firestore();

  // Rename cln_ users
  for (const [code, name] of Object.entries(realNames)) {
    const email = `vip_${code.toLowerCase()}@gmail.com`;
    const userSnap = await db.collection('users').where('email', '==', email).get();
    if (!userSnap.empty) {
      await userSnap.docs[0].ref.update({
        fullName: name
      });
    }
  }
  console.log(`[${projectTag}] Renamed cln_ users to real names`);

  // Count
  const uSnap = await db.collection('users').get();
  const vips = uSnap.docs.filter(d => d.data().vipPlan);
  const pSnap = await db.collection('payments').where('paymentFor', '==', 'vip_upgrade').where('status', '==', 2).get();
  
  console.log(`[${projectTag}] VIP_ACCOUNTS: ${vips.length}`);
  console.log(`[${projectTag}] VIP_UPGRADES: ${pSnap.size}`);
}

async function main() {
  await renameAndCount('old', oldServiceAccount);
  await renameAndCount('new', newServiceAccount);
  process.exit(0);
}

main();
